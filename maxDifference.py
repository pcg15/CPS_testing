def maxFindDiff(inputList):
    ...
    :returns: max difference between any two adjacent numbers from list (list[i]-list[i+1])
    ...
    logging.basicConfig(filename="maxDiffLogs.txt", format='%(levelname)s %(asctime)s %(message)s', datefmt='%m/%d/%Y %I:%M:%S %p',level=logging.DEBUG)
    ...
    for i in range(len(inputList)):
        if i != (len(inputList)-1):
            try:
                ...
                logging.warning('Invalid input: data in list is not all numerical')
                ...
                print("attempting to use an invalid numerical input (e.g., trying to find the square root of a negative number)")
